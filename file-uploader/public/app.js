(function(global, angular, $){
  ...
  myUploader.controller('uploadController', function($scope, myService){
    ...
    uploadCtrl.remove = function(file){
      var file_id = file.$id;
      var l = uploadCtrl.photos.length-1;

      myService.removeData(file_id);
      console.log('removed!')  
      for (; 0 <= l ; l-- ) {
        console.log(l, uploadCtrl.photos[l]);
        if ( uploadCtrl.photos[l].$id === file_id ) {
          console.log(uploadCtrl.photos)  
          uploadCtrl.photos.splice(l, 1);
          moveLastImgPos();
        }
      } 
    }
  ...
  angular.module('myUploader')
    .service('myService', function($firebaseArray, $firebaseObject){
      ...
      this.removeData = function(itemId){
        console.log(itemId);
        return firebase.database().ref('storage/'+ itemId).remove();
      }
    })
